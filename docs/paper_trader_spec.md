...
**REST bootstrap** requires JWT authentication. **WebSocket candles channel is public
— no auth required.** Sending a JWT in the WS subscribe message causes an
`{"type":"error","message":"authentication failure"}` response and no candle data
will flow. Subscribe without auth fields for the candles channel.

Coinbase Advanced Trade uses **JWT authentication** for REST. For each REST request,
generate a JWT signed with your EC private key and include it as a Bearer token.
...
```
COINBASE_KEY_NAME=your_api_key_name       # format: "organizations/.../apiKeys/..."
COINBASE_PRIVATE_KEY=your_api_secret      # EC private key (-----BEGIN EC PRIVATE KEY-----)
```

**Going live (real order execution):** The private `user` WebSocket channel (order
fills, account balances) does require JWT auth. The WS JWT format differs from REST
— it omits the `uri` claim. You will need to confirm the exact format with Coinbase
docs before building a live trader. The REST API (for placing orders) uses the same
JWT format as the candle bootstrap above.
...
**Subscribe message** (no auth needed — candles is a public channel):
```json
{
  "type": "subscribe",
